...
Если команду сделать внешней, то теряется смысл - буде запускать дополнительный процесс, который должен управлять другим процессом - это не логично, проще менять состояние текущего процесса (терминала), находясь в нем (так как сам процесс и есть интерпретатор) без запуска других приложений (внешних команд).
...
* echo - выводит текст после компанды на дисплей
* tee - конкретно перенаправляет из stdin в stdout или файл

Тем самым результат выполнения echo не из под root 
не даст записать в каталог резeльтат выполнения, а если 
этот stdout от echo перенаправить на stdin  tee, который запущен под sudo, то он успешно выполнится.

Иными словами, в первом случаим передаем stdout echo на stdin нового файла, что бы записать его, но прав не хватате для этого.
Во втором же случаи, мы передаем результат stdout echo пайпом на stdin tee, запущенного из под sudo.
